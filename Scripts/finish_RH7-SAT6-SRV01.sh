...
yum -y install git
git clone https://github.com/cloudxabide/matrix.lab; cd matrix.lab

# I no longer need/use this
#sed -n '/# HOSTS - BEGIN/,/# HOSTS - END/p' zion/etc/dhcp/dhcpd.conf > /tmp/dhcpd.hosts
...
yum -y install satellite chrony
...
# if you lose/forget the password
## foreman-rake permissions:reset

# Save the manifest file in ~ - then upload it
hammer subscription upload --file $(find ~ -name "*$MATRIXLABS*.zip" | tail -1) --organization="${ORGANIZATION}" 
...
do
  ...
  do 
    echo "hammer repository-set enable --organization=\"${ORGANIZATION}\" --basearch='x86_64' --releasever=${VER} --product=\"${PRODUCT}\" --id=\"${REPO}\" "
    hammer repository-set enable --organization="${ORGANIZATION}" --basearch='x86_64' --releasever=${VER}  --product="${PRODUCT}" --id="${REPO}"
  done
done
...
REPOS="9641 9662 10269 11502" # Satellite Tools 6.5/6.6/6.7/6.8/6.9
for REPO in $REPOS
do
  echo; echo "NOTE:  Enabling (${REPO}): `grep $REPO ~/hammer_repository-set_list-"${PRODUCT}".out | cut -f3 -d\|`"
  hammer repository-set enable --organization="${ORGANIZATION}" --basearch='x86_64' --product="${PRODUCT}" --id="${REPO}"
